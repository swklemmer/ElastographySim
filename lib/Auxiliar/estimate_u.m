...
% Create dimensions for the estimation and correlation
est_x = (0:N_x-1) * hop_x * dx;
est_z = ((0:N_z-1) * hop_z + ((w_z - 1) / 2)) * dz;

corr_x = (-w_x+1:w_x-1) * 2 * dx;
corr_z = (-w_z+1:w_z-1) * 2 * dz;
...
for t = 1:size(sonograms, 1)-1
    ...
    for x = 1:N_x
        for z = 1:N_z
            ...
            u_x_est(t, x, z) = - corr_x(max_x) - fine_dim(max_dx) * 2 * dx;
            u_z_est(t, x, z) = - corr_z(max_z) - fine_dim(max_dz) * 2 * dz;
