...
cell_density = 10;  % [scatterers/cell]
density_list = ones(20, 1) * cell_density;
errors = zeros(length(density_list), 2);
est_displacements = zeros(length(density_list), 200, 200);

for i = 1:length(density_list)
    % Place scatterers
    % Scatterer density calculation
    cell_vol = resolution_cell(sim_data, 0);
    density = density_list(i) / (cell_vol * 1e9); % [scatterers/mm^3]
    ...
    est_displacements(i, 1:length(est_x), 1:length(est_z)) = u_est_filt;
...
save(sprintf("out_experiments/random_experiment_d%.1f_win_%.2f.mat",...
    density, win_len*1e3), 'errors', 'density_list', 'est_displacements')

fig = figure();
fig.Position = [0, 500, 300, 250];
plot(errors(:, 1))
hold on
plot(errors(:, 2))
grid on
yline(mean(errors(:, 2)), '--')
legend(["Pre filter"; "Post fillter"; "PF mean"])
ylim([0 0.1])
title(sprintf("Random samples of D=%d (d=%.2f)", cell_density, density))
ylabel("Mean Abs. Error in phantom")
saveas(fig, sprintf("out_fig/random_scatteirng_d%.1f_win%.2f.fig",...
       density, win_len*1e3))
